...
if ($rid) {
	$dbh->do(
		"UPDATE AWP_CaseResource SET Status=3
              WHERE Space='$space_name' AND value='$rid'"
	) or die "Cannot release resource for $rid\n";

	$dbh->disconnect;
	print "resource[$space_name, $rid] was released\n";
	exit 0;
}
...
if ($get && $space_name) {
	$sth = $dbh->prepare(
		"SELECT Value FROM AWP_CaseResource
                          WHERE Space='$space_name' AND Status=3 LIMIT 1"
	);
	$sth->execute;
	...
	$dbh->do(
		"UPDATE AWP_CaseResource SET Status=2,JobID = '$jobid',User='$runUser'
              WHERE Space='$space_name' and value='$value'"
...
=head1 SYNOPSIS
    get resource
        handle_br_mgt0_subnet.pl --get --jobid '2013-0828' --user 'admin'
        handle_br_mgt0_subnet.pl -g -j '2013-0828' -u 'admin'
    Release resource
...
=item B<--name|-n>
Get a space name and return the resource id
