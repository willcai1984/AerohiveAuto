...
#Name: Joe Nguyen
#Description: 
# This utilily is used to launch multiple processes
# 
#
#
#--------------------------------------------------------
...
sub parseProcessTable{
	...
	foreach $index ( sort { $a <=> $b } keys %{$data->{process}} ) {
...
#		$log->info( "1a==> KK == $kk ; data=$data->{process}{$index}{$kk} ");
		if ( $kk =~ /loop/ ) {
#		    $log->info( "1==> KK == $kk ; data=$data->{process}{$index}{$kk} ");
		    #convert to number 
		    if ( $data->{process}{$index}{$kk} =~ /[a-zA-Z]/ ) {
			$ptr->{$index}{$kk} = subEnv ($profFile,$data->{process}{$index}{$kk});
		    }
		}
	    }
	}
...
#	    $log->info( " ============      KK == $kk ; data=$data->{process}{$kk} ");
	    if ( $kk =~ /loop/ ) {
#		$log->info( "2==> KK == $kk ; 'data=$data->{process}{$kk}' ");
		#convert to number 
		if ( $data->{process}{$kk} =~ /[a-zA-Z]/ ) {
		    $ptr->{$index}{$kk} = subEnv ($profFile,$data->{process}{$kk});
		}
...
sub parsingXmlProcessFile {
    ...
    $log->info(" Starting Parse process table ");
...
sub printProcTbl {
    ...
    print $handle (" $data\n");
    print $handle ("$temp\n");
    foreach  $index ( sort { $a <=> $b } keys  %{$ptrProc} ) {
...
mltprocess.pl - This is the utility to launch multiple processes at the same time. All the processes need to be defined in the xml form. 
