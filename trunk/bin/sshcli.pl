...
my %userInput = (
	...
	"logOff"       => 0,
	"noprint"      => 1,
	"errtable"     => [ "Login failed due to a bad username or password", "parser error :", ],
);
...
$rc = GetOptions(
	...
	"o=s"    => \$userInput{timeout},
	"t=s"    => \$userInput{outputfile},
	"u=s"    => \$userInput{user},
	"c"      => \$userInput{nocredential},
	...
	"i=s" => sub {
		if ( exists $userInput{insert}[0] ) { push( @{ $userInput{insert} }, $_[1] ); }
		else                                { $userInput{insert}[0] = $_[1]; }
	},
);
...
if ( $userInput{noprint} ) {
	if ( $limit != 0 ) {
		foreach $line ( @{ $userInput{commands} } ) { print "$line \n"; }
	}
}
...
sshcli.pl - This is used to access remote server and sequentially execute command after loggin the target then exit
...
[B<-t> I<output file to save file >]
[B<-l> I<log file directory>]
[B<-o> I<time out for each command executed by sshcli.pl >]
...
1. The following command is used to loggin to esx006 with user=root and password=password and execute "ls ..." and save the output to "test123.txt" in the directory /tmp
         sshcli.pl -t test123.txt -d esxd006 -u root -p password  -v "ls -alt /var/spool/cron" -l /tmp


2. The following command is used to loggin to esx006 with user=root and no password=password and execute "ls ..." and save the output to "test123.txt" in the directory /tmp
         sshcli.pl -t test123.txt -d esxd006 -u root -c  -v "ls -alt /var/spool/cron" -l /tmp
