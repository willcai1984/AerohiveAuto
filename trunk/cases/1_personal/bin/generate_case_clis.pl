...
sub print_cli_switch{
    my $img = shift;
    $output = <<END;
needUpgradeDUT=true
imgName=$img
sw1.cli=-v "no interface eth1/1 shutdown" -v "no interface eth1/21 shutdown" -v "no interface eth1/22 shutdown" -v "no interface eth1/23 shutdown" -v "no interface eth1/24 shutdown"
