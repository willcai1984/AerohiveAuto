...
dut1.imgName=$dut1_img
dut1.shellpwd=2iSNFbhwQlp2JHoO
END
    }
    else{
...
dut1.imgName=$dut1_img
dut1.shellpwd=GLa5MB6M0akKkpQv
...
dut2.imgName=$dut2_img
dut2.shellpwd=GgUnVbAjFF0VCe3w
...
sub switch_print_cli{
    ...
    if($dut1_img || $dut2_img || $dut3_img || $dut4_img){
        $output = <<END;
needUpgradeDUT=true
sw1.cli=-v "no interface eth1/1 shutdown" -v "no interface eth1/21 shutdown" -v "no interface e1/22 shutdown" -v "no interface e1/23 shutdown" -v "no interface e1/24 shutdown"
dut1.consname=hzswtb8_sys-SR2024
dut1.imgName=$dut1_img
dut1.shellpwd=mFE7VdxxUHWpWBlC
dut2.consname=hzswtb8_sys-SR2024P
dut2.imgName=$dut2_img
dut2.shellpwd=YMEMscmoIm2k7NJL
dut3.consname=hzswtb8_sys-SR2124P
dut3.imgName=$dut3_img
dut3.shellpwd=i6GEAFrsqSP4iVPe
dut4.consname=hzswtb8_sys-SR2148P
dut4.imgName=$dut4_img
dut4.shellpwd=RaiXebKr5nHLttB8
END
    }
    else{
        $output = <<END;
...
given($duttype){
	...
	when (m/switch/i){
		...
		$sr2124p = $snake ? $snake : $sw unless $sr2124p;
		$sr2148p = $snake ? $snake : $sw unless $sr2148p;
        switch_print_cli($sr2024, $sr2024p, $sr2124p, $sr2148p);
	}
}
