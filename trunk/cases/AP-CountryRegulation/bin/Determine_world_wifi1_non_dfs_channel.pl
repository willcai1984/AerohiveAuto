#!/usr/bin/perl
################################################################################
# File Name: Determine_world_wifi1_non_dfs_channel.pl
...
################################################################################
## Function: 
## Usage: perl Determine_world_wifi1_non_dfs_channel.pl debug.log wifi_channel.log wifi1
...
my $j;
my @check_value;
my @world_dfs_channel = ('52','56','60','64','100','104','108','112','116','120','124','128','132','136','140');
my @world_none_dfs_channel = ('36','40','44','48');
...
for($i = 0; $i < @world_none_dfs_channel; $i ++) {
	if($channel == $world_none_dfs_channel[$i]){
		print "wifi1 selected non-dfs channel";
		exit 0;
	}
}

for($j = 0; $j < @world_dfs_channel; $j ++){
	if($channel == $world_dfs_channel[$j]){
		last;		
	}
	elsif($j == @world_dfs_channel - 1) {
		if($channel != $world_dfs_channel[$j]){
