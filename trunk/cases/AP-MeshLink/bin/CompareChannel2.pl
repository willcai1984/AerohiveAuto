#!/usr/bin/perl
################################################################################
# File Name: CompareChannel2.pl
# Description: this script is used to check the Scan out of the channel 
#			   and log in monitoring the same with channel
################################################################################
## Usage: perl CompareChannel2.pl channel1.log channel2.log wifi0/wifi1
...
while($line = <FILECHECKED>){
    if($line =~ /$interface_name.*?:\s+monitoring\s+raw\s+frames\s+for\s+channel\((\d+)\)/i){
        push @arrayb, $1;
    }
}
close FILECHECKED;
