...
from webui.apollo_gui.xpath import *
import time
...
class CLOUD(object):
    ...
    def to_menu(self, menu_name):
        self.w.wait_until_element_displayed(Home.get(str(menu_name)))
        self.w.info('handle to menu => ' + str(menu_name), True)
        self.w.click(Home.get(str(menu_name)))
        ...
        try:
            ...
            self.w.info('displayed the menu [ %s ] successful! ' %MenuSuccessfulPage.get(str(menu_name)))
        except Exception, e:
